...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef *pcdHandle)
{
    if (pcdHandle->Instance == USB_OTG_FS)
    {
        ...
        /* D1 D2 D3 D4 D5 D6 D7 */
        HAL_GPIO_DeInit(GPIOB, GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_5 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13);
        /* D0 */
...
static rt_err_t _init(rt_device_t device)
{
    ...
    HAL_PCD_Init(pcd);
    HAL_PCDEx_SetRxFiFo(pcd, 0x200);
    HAL_PCDEx_SetTxFiFo(pcd, 0, 0x200);
    HAL_PCDEx_SetTxFiFo(pcd, 1, 0x200);
    HAL_PCDEx_SetTxFiFo(pcd, 2, 0x200);
    HAL_PCDEx_SetTxFiFo(pcd, 3, 0x200);
    HAL_PCD_Start(pcd);
    return RT_EOK;
}
